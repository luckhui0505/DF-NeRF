...
def sobel_edges(image_tensor):
    # image_tensor: torch.Tensor, shape [1, 3, H, W] or [batch_size, 3, H, W]

    # 确保输入是 4 维张量，形状为 [batch_size, 3, H, W]
    if image_tensor.ndimension() == 3:
        image_tensor = image_tensor.unsqueeze(0)  # 转为 [1, 3, H, W]
    
    # 将 RGB 转为灰度图 (1通道)
    gray_image = rgb_to_grayscale(image_tensor)  # shape: [batch_size, 1, H, W]
    ...
    return edges.squeeze(0)  # 去掉 batch 维度，返回 [H, W]

def sobel_edges2(image):
    # 确保图像是 3D 或 4D 张量
    if image.dim() == 2:
        # 如果是 2D 张量，增加批次维度和通道维度，变成 [1, 1, height, width]
        image = image.unsqueeze(0).unsqueeze(0)
    elif image.dim() == 3:
        # 如果是 3D 张量，增加批次维度，变成 [1, channels, height, width]
        image = image.unsqueeze(0)
    
    # Sobel 卷积核（分别对应 x 和 y 方向的边缘）
    ...
    # 扩展 Sobel 卷积核为 4D 张量：形状 (out_channels, in_channels, height, width)
    ...
    # 计算梯度的幅值（可以使用 L2 范数或简单的平方和开方）
    edge_output = torch.sqrt(grad_x**2 + grad_y**2)

    return edge_output

# 定义边缘感知损失函数
def edge_aware_loss(output, target):
    edge_output = sobel_edges2(output)
    edge_target = sobel_edges2(target)
    edge_loss = F.l1_loss(edge_output, edge_target)
    return edge_loss
    

def save_edge_features(edge_features, step, save_dir, prefix="edge"):
    import os
    import numpy as np
    import imageio
    os.makedirs(save_dir, exist_ok=True)
    
    #print(f"Original shape of edge_features: {edge_features.shape}")
    
    # 检查并适配 edge_features 的形状
    if edge_features.ndim == 3 and edge_features.shape[0] == 1:
        # 如果是 [1, H, W]，直接去掉第一个维度
        edge_np = edge_features.squeeze(0).cpu().numpy()
    else:
        raise ValueError(f"Unsupported edge_features shape: {edge_features.shape}")
    ...
    # 保存图像
    file_path = os.path.join(save_dir, f"{prefix}_step{step:06d}.png")
    imageio.imwrite(file_path, edge_np)
   # print(f"Saved edge features at step {step} to {file_path}")
...
def train():
    ...
    optimizer = torch.optim.Adam(params=optim_params,
                                 ...
                                 betas=(0.9, 0.999)) 
     ##改改                                
    optimizer2 = torch.optim.Adam(params=kernelnet.linears4.parameters(),
                                 lr=2e-4,
                              betas=(0.9, 0.999)) 
    ##改改                         
    start = 0
    ...
    for i in range(start, N_iters):
        ...
        # 如果有其他损失（如 img_loss0），继续加到总损失中,额外的渲染损失
        ...
        # 计算边缘感知损失
        edge_loss = edge_aware_loss(rgb, target_rgb)
        # 加入边缘感知损失到总损失中，使用一个可调的权重
        edge_loss_weight = 0.03	# 可以根据实验调整这个权重
        loss = loss + edge_loss_weight * edge_loss
        
 	###############################################gai###############################################
	# 如果有额外的损失（如 extra_loss），加到总损失中
        ...
        loss.backward()
        for p in kernelnet.linears4.parameters():
...
###############################################gai###############################################                    
                ...
                for rgb_idx, rgb in enumerate(rgbs):
                   # 对 RGB 图像进行边缘特征提取
                   edge_save_dir = os.path.join(basedir, expname, "edge_features")  # 设置边缘特征保存目录
                   # 计算当前输出和目标图像的边缘特征
                  # rgb_tensor = torch.tensor(rgb).permute(2, 0, 1).unsqueeze(0).float()  # 转为 [1, 3, 400, 600] 的 Tensor
                   rgb_tensor = rgb.clone().detach().permute(2, 0, 1).unsqueeze(0).float()

                   edge_output = sobel_edges(rgb_tensor)

                   # 保存边缘特征图像
                   save_edge_features(edge_output, i, edge_save_dir, prefix="output")
